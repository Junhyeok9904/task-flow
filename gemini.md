...
## 6. 🚨 트러블슈팅 및 알려진 이슈 (Troubleshooting)
* Windows 배치 스크립트 실행 에러(CRLF 줄바꿈 이슈 등)와 같은 프로젝트 관련 트러블슈팅 내역 및 에러 로그는 [`error.md`](./error.md) 파일을 참조하세요.

---

## 7. 🤖 AI 어시스턴트 작업 원칙 (Working Principles)
* **에러 및 실수 기록 (Error Tracking)**: 작업 중 발생한 문제, 버그, 또는 AI의 실수(예: 파일 인코딩/줄바꿈 문제)는 즉시 해결책과 함께 `error.md`에 상세히 기록합니다.
* **재발 방지 (Prevention)**: 코드를 수정하거나 새로운 스크립트를 작성할 때, 항상 `error.md`와 이 `GEMINI.md`의 트러블슈팅 내역을 사전 검토하여 동일한 실수를 반복하지 않습니다.
* **사용자 의도 동기화 (Alignment)**: 사용자의 의도와 피드백을 프로젝트 가이드에 지속적으로 반영하여, 지시사항에 맞게 정확하게 움직일 수 있도록 컨텍스트를 유지합니다.
* **태스크 보드 자동 업데이트 (Auto-sync Task Board)**: 등록된 버그나 기능을 수정/완료했을 경우, 별도의 지시가 없더라도 즉시 `data/tasks.json`을 업데이트하여 해당 태스크의 상태를 완료(`completed`)로 변경합니다.
